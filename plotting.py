...
def generate_final_plot(
    ...
    ax_loss.set_ylim(bottom=-10, top=100)
    return fig
...
class RealTimePlot:
    ...
    def _collect_and_process_sample(self) -> Sample:
        ...
        if self.current_ap is not None and ap_mac != self.current_ap:
            self.ap_changes.append(APChange(time=elapsed, name=ap_name))
            console.print(f"[bold yellow]Cambio de AP -> {ap_name}[/bold yellow]")
    ...
    def _update_artists(self, sample: Sample) -> None:
        ...
        for ax in self.axes.values():
            ...
            _draw_ap_change_annotations(ax, self.ap_changes)
    ...
    def stop(self) -> None:
        ...
        time.sleep(0.5)  # Espera breve para asegurar que el hilo se detiene
        if self.iperf_client:
    ...
    def save_plot_image(self, new_size: Optional[Tuple[int,int]] = None) -> bool:
        fig_to_save = generate_final_plot(
            self.interface, self.samples, self.ap_changes,
    ...
    def save_plot_data_csv(self) -> bool:
        ...
        metadata = {
            ...
            'AP_Changes_JSON': json.dumps([
                {'time': c.time, 'name': c.name} for c in self.ap_changes
            ]),
        }
